...
def get_gradient_histogram_vec_from_patch(
    ...
    for i in range(n//4):
      for j in range(n//4):
        histogram = np.histogram(flattened_splitted_oris[i][j], orientation_number, 
                    (-np.pi, np.pi), False, flattened_splitted_mags[i][j])
        wgh.append(histogram[0])
    wgh = np.array(wgh).reshape(orientation_number*(n//4)**2,1)
...
def get_feat_vec(
    ...
    r = int(r)
    c = int(c)
    window_mags = magnitudes[r-(feature_width//2 - 1):r+(feature_width//2 + 1),c-(feature_width//2 - 1):c+(feature_width//2 + 1)]
    window_oris = orientations[r-(feature_width//2 - 1):r+(feature_width//2 + 1),c-(feature_width//2 - 1):c+(feature_width//2 + 1)]
    fv = get_gradient_histogram_vec_from_patch(window_mags,window_oris, orientation_number)
...
def get_SIFT_descriptors(
    ...
    for i in range(k):
      fvs[i] = (get_feat_vec(X[i], Y[i], magnitudes, orientations))
...
def get_sift_features_vectorized(
    ...
    magnitudes, orientations = get_magnitudes_and_orientations(Ix, Iy)
    (k,) = X.shape
    fvs = np.empty((k-800, (feature_width//4)**2*orientation_number, 1))
    for i in range(k-800):
      fvs[i] = (get_feat_vec(X[i], Y[i], magnitudes, orientations, feature_width, orientation_number))
    (k, feat_dim,_) = fvs.shape
    fvs = fvs.reshape((k, feat_dim))
