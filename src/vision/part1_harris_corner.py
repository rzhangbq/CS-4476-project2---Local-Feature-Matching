...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    horizontal_sobel_filter = torch.Tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]).reshape((1, 1, 3, 3))
    vertical_sobel_filter = torch.Tensor([[-1, -2, -1], [0, 0, 0], [1, 2, 1]]).reshape((1, 1, 3, 3))
    image = torch.from_numpy(image_bw).reshape((1, 1, image_bw.shape[0], image_bw.shape[1]))
    Ix = nn.functional.conv2d(image, horizontal_sobel_filter, padding=1)[0, 0]
    Iy = nn.functional.conv2d(image, vertical_sobel_filter, padding=1)[0, 0]
    Ix = np.asarray(Ix)
    Iy = np.asarray(Iy)
    return Ix, Iy
...
def compute_harris_response_map(
    ...
    R = (S_xx) * (S_yy) - S_xy**2 - alpha*(S_xx+S_yy)**2
...
def nms_maxpool_pytorch(
    ...
    max_pool = torch.nn.MaxPool2d(ksize, stride=1, padding=(ksize - 1) // 2, return_indices=True)
    (m, n) = R.shape
    R = R.reshape(1, m, n)
    maxpooled_R, indexes = max_pool(R)
    maxpooled_R[maxpooled_R!=R] = -1
    maxpooled_R = maxpooled_R.reshape([m*n])
    indexes = indexes.reshape([m*n])

    maxpooled_R_with_index = torch.stack((maxpooled_R, indexes),1)
    maxpooled_R_with_index = torch.unique(maxpooled_R_with_index, sorted = True, dim = 0)
    maxpooled_R_with_index = torch.flip(maxpooled_R_with_index, [0])
    maxpooled_R_with_index = maxpooled_R_with_index[:k]

    x = [element[1]%n for element in maxpooled_R_with_index]
    x = np.array(x)
    y = [element[1]//n for element in maxpooled_R_with_index]
    y = np.array(y)
    confidences = [element[0] for element in maxpooled_R_with_index]
...
def remove_border_vals(
    ...
    x_valid = (x >= 11) & (x <= img_w - 13)
    y_valid = (y >= 11) & (y <= img_h - 13)
